...
function getCurrentPrayer() {
    ...
    for (let i = 0; i < todaysPrayerTimes.length; i++) {
        if (currentTime < todaysPrayerTimes[i] ||
 ...
 */
async function safePush(array, element) {
    if (element !== null) {
        array.push(element);
    }
}
...
function updateInfobox() {
    ...
    if (announcements.length > 0) {
        ...
        if (announcements[0]['startDate'] <= todayWithoutTime &&
...
function autoSizeText() {
    ...
    if (fontSizeImportantDateLeft && fontSizeImportantDateRight) {
        const minFontSize = Math.min(parseInt(fontSizeImportantDateLeft, 10), parseInt(fontSizeImportantDateRight, 10)) + 'px';
